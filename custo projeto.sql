...
FROM(SELECT CODIGO, PROJETO, SUM(PLANEJADO) PLANEJADO, SUM(EFETIVO) EFETIVO,SUM(PLANEJADO-EFETIVO) Saldo, SUM(PRJ_PLAN+PRJ_EFE+PLANEJADO) ID, INVID, GERENTE, MANAGERID
...
FROM(SELECT CODIGO, PROJETO, SUM(PLANEJADO) PLANEJADO, SUM(EFETIVO) EFETIVO,SUM(PLANEJADO-EFETIVO) Saldo, SUM(PRJ_PLAN+PRJ_EFE) ID, INVID
...
(SELECT ca.CODPROJETO CODE, INV.NAME NAME, SUM(SL.SLICE) SLICE, SUM(SL.PRJ_OBJECT_ID) PRJ_OBJECT_ID, INV.ID INV_ID, MONTH(SL.SLICE_DATE) Mes
FROM
NIKU.ODF_CA_CONTROLEFINAN CON
INNER JOIN NIKU.ODF_SL_5025018 SL ON CON.ID = SL.PRJ_OBJECT_ID
INNER JOIN NIKU.INV_INVESTMENTS INV ON CON.ODF_CNCRT_PARENT_ID = INV.ID
LEFT JOIN NIKU.ODF_CA_PROJECT AS CA ON INV.ID = CA.ID
WHERE INV.IS_ACTIVE = 1
GROUP BY ca.CODPROJETO, INV.NAME,INV.ID, SL.SLICE_DATE
) SL

FULL OUTER JOIN 


(SELECT ca.CODPROJETO CODE, INV.NAME NAME, SUM(SL.SLICE) SLICE, SUM(SL.PRJ_OBJECT_ID) PRJ_OBJECT_ID, INV.ID EFE_INVID, MONTH(SL.SLICE_DATE) Mes
FROM
NIKU.ODF_CA_CONTROLEFINAN CON
INNER JOIN NIKU.ODF_SL_5025020 SL ON CON.ID = SL.PRJ_OBJECT_ID
INNER JOIN NIKU.INV_INVESTMENTS INV ON CON.ODF_CNCRT_PARENT_ID = INV.ID
...
GROUP BY SL.CODE, SL.NAME, EFE.NAME, EFE.CODE,INV_ID, EFE_INVID) A

GROUP BY CODIGO, A.PROJETO, INVID) final
